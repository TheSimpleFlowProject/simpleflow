...
def issue_rewriter(issue_content, comment):
    ...
    Output: Return only the issue in Markdown, do not wrap it in triple backticks and keep it clear, just the issue content SHOULD be returned.
    The input is the original issue and you've got to modify it by using FeedbackOfInput with the feedback/comment of the developer
    Input: {issue_content}
    FeedbackOfInput: {comment}
    """
...
def lambda_handler(event, context):
    ...
    if "issue_content" not in body or "issue_comment" not in body:
        return {
            'statusCode': 404,
            'body': "not found"
        }

    issue_body = issue_rewriter(body['issue_content'], body['issue_comment'])
    return {
        'statusCode': 200,
        'body': {
